"""Concise tests for DynSCM phase-3 stability guards."""
...
def test_guard_a_support_and_budget_invariants(dynscm_api):
    config_mod, graph_mod, stability_mod = dynscm_api
    cfg = config_mod.DynSCMConfig()
...
def test_guardc_rescale_lag_block_reduces_radius(dynscm_api):
    ...
    lag_rescaled, scale, rho_after = stability_mod.guardc_rescale_lag_block(
        lag, delta=0.95
    )
...
def test_guardc_enabled_in_sampling_enforces_delta(dynscm_api):
    config_mod, graph_mod, stability_mod = dynscm_api
    cfg = config_mod.DynSCMConfig.from_dict(
        {
            "enable_guardc_rescale": True,
            "guardc_delta": 0.90,
            "col_budget_min": 0.90,
            "col_budget_max": 0.99,
        }
    ...
    assert np.all(sample.lag_spectral_radii <= cfg.guardc_delta + 1e-6)

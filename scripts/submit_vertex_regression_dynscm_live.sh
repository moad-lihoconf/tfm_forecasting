...
usage() {
  cat <<'USAGE'
Usage: scripts/submit_vertex_regression_dynscm_live.sh --research-profile NAME [OPTIONS] [-- <extra pretrain args>]

Submit a Vertex AI custom training job for live DynSCM regression research.

Options:
  --research-profile NAME  Named live DynSCM research profile (required)
  --run-name NAME          Logical run name (default: vertex-reg-live-<timestamp>)
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    --research-profile)
      RESEARCH_PROFILE="$(require_value "$1" "${2-}")"
      shift 2
      ;;
    --research-profile=*)
      RESEARCH_PROFILE="$(require_value --research-profile "${1#*=}")"
      shift
      ;;
    --run-name)
...
if [[ -z "$RESEARCH_PROFILE" ]]; then
  echo "Error: --research-profile is required." >&2
  exit 2
fi
...
if [[ -z "$RUN_NAME" ]]; then
  RUN_NAME="vertex-reg-live-$(date +%Y%m%d-%H%M%S)"
fi
if [[ -z "$DISPLAY_NAME" ]]; then
  DISPLAY_NAME="tfm-regression-live-${RUN_NAME}"
fi
...
TRAIN_ARGS=(
  /app/pretrain_regression_dynscm_live.py
  ...
  "--debug_train_trace_json=${METADATA_URI}/train_trace.json"
  "--debug_train_trace_summary_json=${METADATA_URI}/train_trace_summary.json"
  "--run_config_json=${METADATA_URI}/run_config.json"
)
if [[ ${#EXTRA_ARGS[@]} -gt 0 ]]; then
  TRAIN_ARGS+=("${EXTRA_ARGS[@]}")
fi

CONFIG_FILE="$(mktemp -t tfm_vertex_job_live_XXXX.yaml)"
{
...
if [[ "$DRY_RUN" -eq 1 ]]; then
  ...
  echo "[dry-run] research_profile: ${RESEARCH_PROFILE}"
